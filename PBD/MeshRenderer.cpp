...
namespace Velvet
{
	...
	void MeshRenderer::SetupLighting(shared_ptr<Material> m_material)
	{
		...
		m_material->SetVec3("spotLight.position", light->position());
		m_material->SetVec3("spotLight.direction", front);
		m_material->SetFloat("spotLight.cutOff", glm::cos(glm::radians(light->innerCutoff)));
		m_material->SetFloat("spotLight.outerCutOff", glm::cos(glm::radians(light->outerCutoff)));

		m_material->SetFloat("spotLight.constant", light->constant);
		m_material->SetFloat("spotLight.linear", light->linear);
		m_material->SetFloat("spotLight.quadratic", light->quadratic);

		m_material->SetVec3("spotLight.color", light->color);
		m_material->SetFloat("spotLight.ambient", light->ambient);
	}
